...
Features
--------
* v1.1.7 is a stable build and segmentation should be of high quality. The code is usable for high throughput with minimal effort with optimization. By default no more than a GRO and XTC (or equivalent) are required for successful segmentation of Martini systems.
...
------------------------------
Luis S. Mayorga and Diego Masone wrote some heartwarming words in their manuscript (`The Secret Ballet Inside Multivesicular Bodies <https://pubs.acs.org/doi/full/10.1021/acsnano.4c01590?casa_token=vTk3uyUw6OEAAAAA%3AjeXpQZlkPH4r2byOc9daaV-ah2Q0p4TQkonu0_PI3r1TSR5bd-gFApr87J_DR0SOFd92YR1tIsDvSA>`_) work about MDVoxelSegmentation. We cannot express how motivating it is for us to read these words of appreciation. If you are reading this README, you are probably into interesting looking lipid morphologies and we can highly recommend reading their work. In their manuscript they write the following in the `Results` section:

*The key feature in this work has been the application to the model autophagosome trajectories of a convenient computational tool by Bruininks et al. (18) that breaks down very complex lipid superstructures in clearly delimited monolayers, otherwise undistinguishable by normal means. This powerful tool tracks the individual lipid leaflets (named segments) in a molecular dynamics simulation unveiling topological changes produced by spontaneous fusion and fission events. Even for the trained eye, without segmentation it is difficult, if not impossible, to figure out the shapes that are confined in the structures that result from the dynamics of the model autophagosome (Figure 1). Despite the use of powerful tools that allow for cross-sectional visualizations (e.g., VMD (35)), inner vesicles are highly irregular and cannot be understood in most cases. Therefore, leaflet segmentation of the model autophagosome has revealed to be a game-changing technique for identifying lipid superstructures and their interactions.*
